...
inputPIR = Input(shape=(1,100))
inputOther = Input(shape=(3,))

model_x1 = LSTM(128, input_shape=(1,100))(inputPIR)
model_x2 = Dense(1, input_shape=(3,))(inputOther)

merged = Concatenate(axis=1)([model_x1, model_x2])

model_x3 = Dense(2, activation='relu')(merged)
...
x_data_test = np.reshape(x_data_test, (-1, 1, 100))
x_data_other_test = np.reshape(x_data_other_test, (-1,3))
print(x_data_other[0])
sys=input("stop")
k = 0
...
print(x_data_other[0])
print(x_data_other[1])
print(x_data_other[2])
#early_stopping = EarlyStopping(monitor='val_loss', min_delta=0, patience=12, verbose=0, mode='auto')
model.fit([x_data,x_data_other], Y_train, epochs=totalepoch, batch_size=512, shuffle='True', validation_data=([x_data_test,x_data_other_test], Y_test), verbose=1, callbacks=[tensorboard])
