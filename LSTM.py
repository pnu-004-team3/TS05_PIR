...
model.add(keras.layers.LSTM(128, return_sequences=True, input_shape=(1,100), activation='relu', recurrent_activation="sigmoid"))
...
X_train, X_test, Y_train, Y_test = train_test_split(DataX, DataY, test_size=0.3, random_state=321, shuffle=True)
...
#model.fit(X_train, Y_train, epochs=total_epoch, batch_size=512, shuffle='True', validation_data=(X_test, Y_test), verbose=1, callbacks=[tensorboard,early_stopping])
model.fit(X_train, Y_train, epochs=total_epoch, batch_size=512, shuffle='True', validation_data=(X_test, Y_test), verbose=1, callbacks=[tensorboard])
